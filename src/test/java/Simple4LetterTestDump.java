 ...
 */
public class Simple4LetterTestDump {
    ...
    public static void main(String[] args) throws Exception {
        int i = 0;
        while (i <= cnt - 1) {
            DumpMetrics dump = new DumpMetrics();
            String ans = socketCmd("192.168.144.111",2181,"dump");
            System.out.println(ans);
            String ls[] = ans.split("\n");
            String key = null;
            String value = null;
            for(String l : ls) {
                if(l.contains(":")) { // is key
                    String ds[] = l.split(":");
                    if(ds.length >= 1) {
                        key = ds[0].trim();
                    }
